...
if not files:
    ...
    if 0 <= selected_index < len(files):
        ...
        paragraph_size = max(1, len(summary_paragraphs) // num_paragraphs)
        paragraphs = [
            " ".join(summary_paragraphs[i:i + paragraph_size])
                for i in range(0, len(summary_paragraphs), paragraph_size)
        ]
